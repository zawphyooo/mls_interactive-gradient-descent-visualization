...
def build_intercept_fig(x: np.ndarray, y: np.ndarray, m: float, b_current: float) -> go.Figure:
    ...
    fig.update_layout(
        title="Diagram 2: Intercept Parameter Space",
...
def build_cost_fig(
    ...
    if use_surface:
        ...
        fig.update_layout(
            title="Diagram 3: Full Cost Surface (m, b)",
            scene=dict(
                xaxis_title="Slope m",
    ...
    fig.update_layout(
        title="Diagram 3: Cost Contour (m, b)",
        xaxis_title="Slope m",
        yaxis_title="Intercept b",
        margin=dict(l=10, r=10, t=40, b=10),
        height=560,
...
st.caption("Simple linear regression with batch gradient descent using SSR as the objective.")
...
st.info(f"Current equation:  y = {m:.4f}x + {b:.4f}")
...
with col2:
    ...
    st.caption(f"Current (Diagram 2): b={b:.3f}, SSR={current_ssr:.3f}")
...
with col3:
    ...
    if use_3d_surface:
        st.caption(
            "Read this as error landscape: height is SSR, red line is optimization path, "
            "orange/cyan curves are local slices through current (m,b), and black vertical line is current SSR height."
        )

st.markdown(
    """
**Update rule used for each step**
